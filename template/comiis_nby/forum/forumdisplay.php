...
<!--{if !(empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle']) && $comiis_list_image}--><!--{eval $leftside=0;$comiis_list_zlb=0;}--><!--{/if}-->
...
<!--{if $_G[forum][banner] && !$subforumonly}--><img src="$_G[forum][banner]" alt="$_G['forum'][name]" style="margin-bottom:10px;"/><!--{/if}-->
<div class="boardnav{if $comiis_list_zlb==1}l{elseif $comiis_list_zlb==2}r{/if}">
<!--{if $comiis_list_zlb>0}-->
<!--{eval $leftside=0;}-->
...
<!--{/if}-->
	<div class="comiis_listinfo cl zhun">
	 <!--{if $comiis_list_xpic==1}-->
		<div class="topico"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]"><img src="<!--{if $_G['forum'][icon]}-->data/attachment/common/$_G['forum'][icon]<!--{else}-->{$_G['style']['styleimgdir']}/comiis_nofid.gif<!--{/if}-->" alt="{$_G['forum'][name]}"></a></div>
	<!--{/if}-->
		...
		<!--{if (!empty($_G[forum][domain]) && !empty($_G['setting']['domain']['root']['forum'])) || $moderatedby || ($_G['page'] == 1 && $_G['forum']['rules'])}-->					
			<!--{if !empty($_G[forum][domain]) && !empty($_G['setting']['domain']['root']['forum'])}-->
				<div class="comiis_infotxt">{lang forum_domain}: <a href="http://{$_G[forum][domain]}.{$_G['setting']['domain']['root']['forum']}" id="group_link">http://{$_G[forum][domain]}.{$_G['setting']['domain']['root']['forum']}</a></div>
			<!--{/if}-->						
		<!--{/if}-->
			<div class="comiis_infotxt"><!--{if !$subforumonly}-->{lang index_today}: $_G[forum][todayposts]<!--{if $_G[forum][todayposts]}--><!--{if $_G[forum][todayposts] < $_G[forum][yesterdayposts]}--><b class="ico_fall">&nbsp;</b><!--{else}--><b class="ico_increase">&nbsp;</b><!--{/if}--><!--{/if}--><span class="pipe">|</span>{lang index_threads}: $_G[forum][threads]<!--{if $_G[forum][rank]}--><span class="pipe">|</span>{lang rank}: <span title="{lang previous_rank}:$_G[forum][oldrank]">$_G[forum][rank]</span><!--{if $_G[forum][rank] <= $_G[forum][oldrank]}--><b class="ico_increase">&nbsp;</b><!--{else}--><b class="ico_fall">&nbsp;</b><!--{/if}--><!--{/if}--></span><!--{/if}--></div>
			<div class="comiis_infotxt">{lang forum_modedby}: <!--{if $moderatedby}-->$moderatedby<!--{else}-->暂无<!--{/if}--></div>
		<!--{if !empty($forumarchive)}-->
	...
	<div id="ct" class="wp cl comiis_rollzbox{if $_G['forum']['allowside'] && $comiis_list_zlb==0} ct2{else} ct2km{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		<!--{if $leftside}-->
			<div id="sd_bdl" class="bdl" onmouseover="showMenu({'ctrlid':this.id, 'pos':'dz'});" style="width:{$_G['setting']['leftsidewidth']}px;margin-left:-{$_G['leftsidewidth_mwidth']}px;{if !$_G['setting']['leftsideopen']}padding-bottom:15px;{/if}">
		...
		<!--{if $_G['forum']['allowside'] && $comiis_list_zlb==0}-->
		<div class="sd">
			...
			<!--{if $_G['forum']['rules']}-->
				<div class="bm">
					...
					<div class="bm_c">
						$_G['forum'][rules]<br><br>
						<!--{if $moderatedby}-->
							<div class="comiis_bzlist cl">
								<h2>{lang forum_modedby}: </h2>
								$moderatedby
							</div>
						<!--{/if}-->
					</div>
...
<div id="comiis_rollbox">	
	<div style="height:95px;overflow:hidden;">	
		<div class="comiis_irbox comiis_irftbtn cl" id="comiis_irftbtn">
			<a href="javascript:;" id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}" class="kmfbzt"></a>
			<a href="javascript:void(0)" class="kmqd" title="签到"><script type="text/javascript">var dfsj = new Date();document.write('周'+'日一二三四五六'.charAt(new Date().getDay())+'<br>'+(dfsj.getMonth()+1)+'月'+dfsj.getDate()+'日');</script></a>
		</div>			
	</div>
	<!--{hook/forumdisplay_side_top}-->
	<!--{if $_G['forum']['rules']}-->
	<div class="comiis_irbox">
		...
		<div id="forum_rules_{$_G[fid]}" class="comiis_bkgz">
			<!--{if $_G['forum']['rules']}-->$_G['forum'][rules]<!--{else}-->暂无公告，请到后台版块规则处添加<!--{/if}-->
		</div>
	</div>
	<!--{/if}-->
