...
<div id="f_pst" class="pl{if empty($_GET[from])} bm bmw{/if}"<!--{if !$close_leftinfo}--> style="margin-top:0;"<!--{/if}-->>
<form method="post" autocomplete="off" id="fastpostform" action="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&extra=$_GET[extra]&replysubmit=yes{if $_GET['ordertype'] != 1}&infloat=yes&handlekey=fastpost{/if}{if $_GET[from]}&from=$_GET[from]{/if}"{if $_GET['ordertype'] != 1} onSubmit="return fastpostvalidate(this)"{/if}>
	...
	<table cellspacing="0" cellpadding="0">
		<tr>
			<td class="pls<!--{if $close_leftinfo}--> comiis_f_pst<!--{/if}-->">
				<!--{if $_G['uid']}-->
					<div class="avatar avtm"><!--{echo avatar($_G['uid'])}--></div>
				<!--{else}-->
					<div class="avatar avtm nologin"><img src="{$_G['style']['styleimgdir']}/comiis_nologin.jpg"></div>
				<!--{/if}-->
	...
	<!--{/if}-->
	<div class="cl">
		<!--{if $_G['uid'] && empty($_GET[from]) && $_G[setting][fastsmilies]}--><div id="fastsmiliesdiv" class="y"><div id="fastsmiliesdiv_data"><div id="fastsmilies"></div></div></div><!--{/if}-->
		<div{if $_G['uid'] && empty($_GET[from]) && $_G[setting][fastsmilies]} class="hasfsl"{/if} id="fastposteditor">
			<div class="tedt{if !($_G[forum_thread][special] == 5 && empty($firststand))} mtn{/if}">
				...
				<div class="area">
					<!--{if $allowfastpost}-->
						<textarea rows="5" cols="80" name="message" id="fastpostmessage" onKeyDown="seditor_ctlent(event, {if $_GET['ordertype'] != 1}'fastpostvalidate($(\'fastpostform\'))'{else}'$(\'fastpostform\').submit()'{/if});" tabindex="4" class="pt"{eval echo getreplybg($_G['forum']['replybg']);}></textarea>
					<!--{else}-->
						<div class="pt hm">
							<!--{if !$_G['uid']}-->
								<!--{if !$_G['connectguest']}-->
