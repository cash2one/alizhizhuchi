		...
		<!--{if $_G['setting']['allowfastreply'] && $post['first'] && $fastpost && $allowpostreply && !$_G['forum_thread']['archiveid'] && $_GET['from'] != 'preview'}-->
			...
			<div id="vfastpost" class="fullvfastpost">				
				...
				<div id="vfastposttb" class="comiis_vfastpost cl">
					...
					<button type="submit" name="replysubmit" id="vreplysubmit" value="true" class="kmvfbtn" onmousemove="this.className='kmvfbtn kmon'" onmouseout="this.className='kmvfbtn'">快捷回复</button>			
				</div>			
			</div>
		...
		<dl id="ratelog_$post[pid]" class="rate">
			...
			<!--{/if}-->
				...
				<!--{if $_G['setting']['ratelogon']}-->
					<table class="ratl">
						<tr>
							<th class="xw1" width="180"><a href="forum.php?mod=misc&action=viewratings&tid=$_G[tid]&pid=$post[pid]" onclick="showWindow('viewratings', this.href)" title="{lang rate_view}">已有<!--{echo count($postlist[$post[pid]][totalrate]);}-->人评分</a></th>
							<!--{loop $post['ratelogextcredits'] $id $score}-->
								<!--{if $score > 0}-->
									<th class="xw1" width="80">{$_G['setting']['extcredits'][$id][title]}</th>
								<!--{else}-->
									<th class="xw1" width="80">{$_G['setting']['extcredits'][$id][title]}</th>
								<!--{/if}-->
							<!--{/loop}-->
						...
						<tbody class="ratl_l">
							...
							<tr id="rate_{$post[pid]}_{$uid}">
								<td>
									<a href="home.php?mod=space&uid=$uid" target="_blank"><!--{echo avatar($uid, 'small');}--></a> <a href="home.php?mod=space&uid=$uid" target="_blank" class="xg1">$ratelog[username]</a>
								</td>
								<!--{loop $post['ratelogextcredits'] $id $score}-->
					...
					<p class="ratc xg1">
						<a href="forum.php?mod=misc&action=viewratings&tid=$_G[tid]&pid=$post[pid]" onclick="showWindow('viewratings', this.href)" title="{lang rate_view}" class="xi1 y">{lang rate_view}</a>
						{lang rate_total}：
						<!--{loop $post['ratelogextcredits'] $id $score}-->
							<!--{if $score > 0}-->
								{$_G['setting']['extcredits'][$id][title]} <span class="xi1">+$score</span>&nbsp;
							<!--{else}-->
								{$_G['setting']['extcredits'][$id][title]} <span class="xi1">$score</span>&nbsp;
							<!--{/if}-->
						<!--{/loop}-->
